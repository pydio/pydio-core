...
require_once(AJXP_INSTALL_PATH."/plugins/access.cuas_smb/3rdparty/autoload.php");
    ...
    protected function getServer($purl){
        if(is_null($this->server)){
            if (Server::NativeAvailable()) {
                $this->server = new NativeServer($purl['host'], $purl['user'], $purl['pass']);
                $this->share = $this->server->getShare(trim($purl['share'], '/'));
            }else {
                self::debug($_SESSION["libsmbclient-php not available, falling back to wrapping smbclient"]);
        ...
        switch ($pu['type']) {
            ...
            case 'share':
                ...
                if(!isset($_SESSION["disk"]) ){
                    $shares = cuas_smb::look ($pu);
                    $_SESSION["disk"] = $shares['disk'];
                }
                $found = false;
                if(is_array($_SESSION["disk"]) && isset($_SESSION["disk"])){
                    $lshare = strtolower($pu['share']);
                    foreach($_SESSION["disk"] as $disk){
                        if ($lshare == strtolower($disk)) {
                            $found = TRUE;
                            $stat = stat(SMB4PHP_SMBTMP);
                            break;
                        }
            ...
            case 'path':
                ...
                try {
                    $statInfo =  $this->getServer($pu)->getStat($pu['path'] );
                    $stat = cuas_smb::addstatcache ($url, $statInfo);
...
###################################################################
# CUAS_SMB_STREAM_WRAPPER - class to be registered for smb:// URLs
###################################################################
            ...
            case 'path':
                ...
                try{
                    ...
                    foreach($dirResult as $item){
                        cuas_smb::addstatcache($url . $item->getName(), $item->stat());
                        $o['info'][$item->getName()] = '';
                    }
                }catch (\Icewind\SMB\Exception\NotFoundException $ex){
...
stream_wrapper_register('smb', 'cuas_smb_stream_wrapper')
