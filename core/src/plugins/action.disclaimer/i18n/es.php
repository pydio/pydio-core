<?php
/*
* Disclaimer plugin for Pydio
*/

//	Spanish translation: Salva Gómez <salva.gomez at gmail.com> 2016
  
$mess=array(
    "1" => "Renuncia de responsabilidad",
    "2" => "Aceptar renuncia de responsabilidad",
    "3" => "Debe aceptar las condiciones de uso generales indicadas a continuación. Por favor, lea y utilice la casilla para aceptar.",
    "4" => "Acepto las condiciones de uso",
);
