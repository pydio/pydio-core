...
class ajxp_confAccessDriver extends AbstractAccessDriver
{
    ...
    public function listRepositories($root, $child, $hashValue = null, $returnNodes = false){
        ...
        if(!isSet($parentRepoId)){
            ...
            if(!$returnNodes){
                ...
                <column messageId="3" attributeName="share_type" sortType="String"/>
        ...
        foreach ($repos as $repoIndex => $repoObject) {
            ...
            if(!empty($shareCenter)){
                ...
                $meta["share_data"] = '';
                $shareData = $shareCenter->findSharesForRepo($repoIndex, $repoObject->getOwner());
                if(count($shareData)){
                    $hashes = array_keys($shareData);
                    $firstKey = $hashes[0];
                    $meta["share_type"] = $repoObject->hasContentFilter() ? "Single File" : "Minisite";
                    $meta["share_data"] = $shareCenter->buildPublicletLink($firstKey);
                    $meta["shared_element_hash"] = $firstKey;
                    $meta["shared_element_type"] = 'repository';
                    $meta["shared_element_parent_repository"] = $repoObject->getParentId();
                    $meta["ajxp_shared_minisite"] = "public";
                }else{
                    $meta["share_type"] = "Workspace";
                    $meta["share_data"] = 'Shared as workspace: '.$repoObject->getDisplay();
                    $meta["shared_element_repository_id"] = $repoIndex;
                    $meta["shared_element_parent_repository"] = $repoObject->getParentId();
                }
            }
    ...
    public function listSharedFiles()
    {
        ...
        $files = $shareCenter->listShares(false, array(0,200)); // we can implement pagination
