...
class serialConfDriver extends AbstractConfDriver {
    ...
    function loadChannel($channelName, $create = false){
        ...
        if(is_file($this->getPluginWorkDir()."/queues/channel-$channelName")){
            ...
            $this->channels[$channelName] = AJXP_Utils::loadSerialFile($this->getPluginWorkDir()."/queues/channel-$channelName");
            return;
        }
        if($create){
            if(!isSet($this->channels)) $this->channels = array();
    ...
    public function publishNodeChange($origNode = null, $newNode = null, $copy = false){
        if($newNode != null) {
            ...
            $message->content = AJXP_XMLWriter::writeNodesDiff(array("ADD" => array($newNode)));
