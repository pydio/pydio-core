...
class PowerFSController extends AJXP_Plugin
{
	...
	public function compress_general ($action, $httpVars, $fileVars, $dir, $selection, $urlBase, $zipper)
	{
			...
			switch ($monproc){
				...
				case "fileoutput" :
					...
					while (!$zipcomplete && !$kill_proc)
					{
						...
						{
							AJXP_Logger::logAction("Abort due logfile has not changed in longer time.");
							$kill_proc = true;
						}
				...
				case "avgspeed" :
					...
					while (!$zipcomplete && !$kill_proc)
					{
						...
						else
						{
							if (!$zipwaiting)
							{
								...
								AJXP_Logger::debug("Waiting for success statement ... 98% is reached");
								$zipwaiting = true;
							}
						}
					...
					if (file_exists($clioutputfile))
						if (strpos(file_get_contents($clioutputfile), $zipsuccess_statement) > 1)
							$zipOK = 1;
					break;
				default:
		...
		$maxwaiting = 60; // Waiting x seconds to give process time to quit
	...
	{ 
		...
		chdir($rootDir);
		$cmd = $this->getFilteredOption("ZIP_PATH")."zip -r ".escapeshellarg($archiveName)." ".implode(" ", $args);
		$fsDriver = AJXP_PluginsService::getInstance()->getUniqueActivePluginForType("access");
		$c = $fsDriver->getConfigs();
