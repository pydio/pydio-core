...
class ConfService
{
    ...
    {
        $inst = PluginsService::getInstance()->findPluginById("boot.conf");
        if (empty($inst)) {
            $inst = PluginsService::getInstance()->softLoad("boot.conf", array());
        }
        return $inst;
    }
    ...
    {
        PluginsService::getInstance()->setPluginUniqueActiveForType("conf", self::getConfStorageImpl()->getName());
    }
    /**
    ...
    public function getMessagesInst($forceRefresh = false)
    {
        ...
        if (!isset($this->configs["MESSAGES"]) && is_file($messageFile)) {
            ...
            VarsFilter::filterI18nStrings($this->configs["MESSAGES"]);
            VarsFilter::filterI18nStrings($this->configs["CONF_MESSAGES"]);
    ...
    public function getRegisteredExtensionsInst()
    {
        if (!isSet($this->configs["EXTENSIONS"])) {
            ...
            $nodes = PluginsService::getInstance()->searchAllManifests("//extensions/extension", "nodes", true);
            $res = array();
            foreach ($nodes as $node) {
