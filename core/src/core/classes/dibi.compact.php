<?php //netteloader=IDataSource,IDibiDriver,IDibiResultDriver,IDibiReflector,dibi,DibiDateTime,DibiObject,DibiLiteral,DibiHashMapBase,DibiHashMap,DibiException,DibiDriverException,DibiPcreException,DibiNotImplementedException,DibiNotSupportedException,DibiConnection,DibiResult,DibiResultIterator,DibiRow,DibiTranslator,DibiDataSource,DibiFluent,DibiDatabaseInfo,DibiTableInfo,DibiResultInfo,DibiColumnInfo,DibiForeignKeyInfo,DibiIndexInfo,DibiEvent,DibiFileLogger,DibiFirePhpLogger,DibiMySqlReflector,DibiMySqlDriver,DibiMySqliDriver,DibiOdbcDriver,DibiSqliteReflector,DibiPdoDriver,DibiPostgreDriver,DibiSqliteDriver,DibiSqlite3Driver
...
new
Exception('dibi needs PHP 5.2.0 or newer.');}interface
IDataSource
extends
...
IDENTIFIER='n',AFFECTED_ROWS='a';const
...
VERSION='2.2.0',REVISION='released on 2014-06-02';const
...
extends
DateTime{function
__construct($time='now',DateTimeZone$timezone=NULL){if(is_numeric($time)){parent::__construct('@'.$time);$this->setTimeZone($timezone?$timezone:new
DateTimeZone(date_default_timezone_get()));}elseif($timezone===NULL){parent::__construct($time);}else{parent::__construct($time,$timezone);}}function
...
DateTimeZone($this->fix[1]));}else{$this->__construct($this->fix[0]);}unset($this->fix);}}abstract
class
DibiObject{private
...
DibiFirePhpLogger($filter),'logEvent');}if(!interface_exists('Tracy\IBarPanel')&&(interface_exists('Nette\Diagnostics\IBarPanel')||interface_exists('IBarPanel'))){$panel=new
...
new
RuntimeException("Cannot open file '$file'.");}$count=0;$delimiter=';';$sql='';while(!feof($handle)){$s=rtrim(fgets($handle));if(substr($s,0,10)==='DELIMITER '){$delimiter=substr($s,10);}elseif(substr($s,-strlen($delimiter))===$delimiter){$sql.=substr($s,0,-strlen($delimiter));$this->driver->query($sql);$sql='';$count++;}else{$sql.=$s."\n";}}if(trim($sql)!==''){$this->driver->query($sql);$count++;}fclose($handle);return$count;}function
...
new
InvalidArgumentException("Unknown key column '$key'.");}}do{$data[(string)$row[$key]]=$row[$value];}while($row=$this->fetch());return$data;}private
function
...
as$key=>$type){if(!isset($row[$key])){continue;}$value=$row[$key];if($value===FALSE||$type===dibi::TEXT){}elseif($type===dibi::INTEGER){$row[$key]=is_float($tmp=$value*1)?$value:$tmp;}elseif($type===dibi::FLOAT){$row[$key]=ltrim((string)($tmp=(float)$value),'0')===ltrim(rtrim(rtrim($value,'0'),'.'),'0')?$tmp:$value;}elseif($type===dibi::BOOL){$row[$key]=((bool)$value)&&$value!=='f'&&$value!=='F';}elseif($type===dibi::DATE||$type===dibi::DATETIME){if((int)$value!==0||substr((string)$value,0,3)==='00:'){$value=new
DibiDateTime($value);$row[$key]=empty($this->formats[$type])?$value:$value->format($this->formats[$type]);}}elseif($type===dibi::BINARY){$row[$key]=$this->getResultDriver()->unescape($value,$type);}}}final
function
...
function
getColumns(){return$this->getInfo()->getColumns();}final
function
...
as$col=>$foo){$len=mb_strlen($col);$maxLen=max($len,$maxLen);}}if($hasColors){echo"\033[1;37m#row: $i\033[0m\n";}else{echo"#row: $i\n";}foreach($row
...
instanceof
DibiDateTime){if((int)$time===0&&substr((string)$time,0,3)!=='00:'){return
NULL;}$time=new
DibiDateTime($time);}return$format===NULL?$time:$time->format($format);}final
function
count(){return
...
__construct(DibiConnection$connection){$this->connection=$connection;$this->driver=$connection->getDriver();$this->identifiers=new
DibiHashMap(array($this,'delimite'));}function
...
clause($clause){$this->cursor=&$this->clauses[self::$normalizer->$clause];if($this->cursor===NULL){$this->cursor=array();}return$this;}function
...
execute($return=NULL){$res=$this->query($this->_export());switch($return){case
dibi::IDENTIFIER:return$this->connection->getInsertId();case
dibi::AFFECTED_ROWS:return$this->connection->getAffectedRows();default:return$res;}}function
...
getForeignKeys($table){$data=$this->driver->query("SELECT `ENGINE` FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = {$this->driver->escape($table,dibi::TEXT)}")->fetch(TRUE);if($data['ENGINE']!=='InnoDB'){throw
new
DibiNotSupportedException("Foreign keys are not supported in {$data['ENGINE']} tables.");}$res=$this->driver->query("
		...
		");$foreignKeys=array();while($row=$res->fetch(TRUE)){$keyName=$row['CONSTRAINT_NAME'];$foreignKeys[$keyName]['name']=$keyName;$foreignKeys[$keyName]['local']=explode(",",$row['COLUMNS']);$foreignKeys[$keyName]['table']=$row['REFERENCED_TABLE_NAME'];$foreignKeys[$keyName]['foreign']=explode(",",$row['REFERENCED_COLUMNS']);$foreignKeys[$keyName]['onDelete']=$row['DELETE_RULE'];$foreignKeys[$keyName]['onUpdate']=$row['UPDATE_RULE'];}return
array_values($foreignKeys);}}class
DibiMySqlDriver
extends
DibiObject
implements
...
connect(array&$config){if(isset($config['resource'])){$this->connection=$config['resource'];}else{DibiConnection::alias($config,'flags','options');$config+=array('charset'=>'utf8','timezone'=>date('P'),'username'=>ini_get('mysql.default_user'),'password'=>ini_get('mysql.default_password'));if(!isset($config['host'])){$host=ini_get('mysql.default_host');if($host){$config['host']=$host;$config['port']=ini_get('mysql.default_port');}else{if(!isset($config['socket'])){$config['socket']=ini_get('mysql.default_socket');}$config['host']=NULL;}}if(empty($config['socket'])){$host=$config['host'].(empty($config['port'])?'':':'.$config['port']);}else{$host=':'.$config['socket'];}if(empty($config['persistent'])){$this->connection=@mysql_connect($host,$config['username'],$config['password'],TRUE,$config['flags']);}else{$this->connection=@mysql_pconnect($host,$config['username'],$config['password'],$config['flags']);}}if(!is_resource($this->connection)){throw
new
...
dibi::DATE:case
dibi::DATETIME:if(!$value
instanceof
DateTime&&!$value
instanceof
DateTimeInterface){$value=new
DibiDateTime($value);}return$value->format($type===dibi::DATETIME?"'Y-m-d H:i:s'":"'Y-m-d'");default:throw
new
...
getResultResource(){$this->autoFree=FALSE;return$this->resultSet===NULL||$this->resultSet->type===NULL?NULL:$this->resultSet;}}class
DibiOdbcDriver
extends
DibiObject
implements
...
instanceof
DateTimeInterface){$value=new
DibiDateTime($value);}return$value->format($type===dibi::DATETIME?"#m/d/Y H:i:s#":"#m/d/Y#");default:throw
new
InvalidArgumentException('Unsupported type.');}}function
...
DibiObject
implements
IDibiDriver,IDibiResultDriver,IDibiReflector{private$connection;private$resultSet;private$autoFree=TRUE;private$affectedRows=FALSE;function
__construct(){if(!extension_loaded('pgsql')){throw
new
DibiNotSupportedException("PHP extension 'pgsql' is not loaded.");}}function
connect(array&$config){if(isset($config['resource'])){$this->connection=$config['resource'];}else{$config+=array('charset'=>'utf8');if(isset($config['string'])){$string=$config['string'];}else{$string='';DibiConnection::alias($config,'user','username');DibiConnection::alias($config,'dbname','database');foreach(array('host','hostaddr','port','dbname','user','password','connect_timeout','options','sslmode','service')as$key){if(isset($config[$key])){$string.=$key.'='.$config[$key].' ';}}}DibiDriverException::tryError();if(empty($config['persistent'])){$this->connection=pg_connect($string,PGSQL_CONNECT_FORCE_NEW);}else{$this->connection=pg_pconnect($string,PGSQL_CONNECT_FORCE_NEW);}if(DibiDriverException::catchError($msg)){throw
new
...
new
DibiDriverException($msg,0);}}if(isset($config['schema'])){$this->query('SET search_path TO "'.$config['schema'].'"');}}function
...
dibi::TEXT:if(!is_resource($this->connection)){throw
new
DibiException('Lost connection to server.');}return"'".pg_escape_string($this->connection,$value)."'";case
dibi::BINARY:if(!is_resource($this->connection)){throw
new
DibiException('Lost connection to server.');}return"'".pg_escape_bytea($this->connection,$value)."'";case
dibi::IDENTIFIER:return'"'.str_replace('"','""',$value).'"';case
...
new
InvalidArgumentException('Unsupported type.');}}function
escapeLike($value,$pos){$value=pg_escape_string($this->connection,$value);$value=strtr($value,array('%'=>'\\\\%','_'=>'\\\\_'));return($pos<=0?"'%":"'").$value.($pos>=0?"%'":"'");}function
...
getResultColumns(){$count=pg_num_fields($this->resultSet);$columns=array();for($i=0;$i<$count;$i++){$row=array('name'=>pg_field_name($this->resultSet,$i),'table'=>pg_field_table($this->resultSet,$i),'nativetype'=>pg_field_type($this->resultSet,$i));$row['fullname']=$row['table']?$row['table'].'.'.$row['name']:$row['name'];$columns[]=$row;}return$columns;}function
getResultResource(){$this->autoFree=FALSE;return
is_resource($this->resultSet)?$this->resultSet:NULL;}function
getTables(){$version=pg_parameter_status($this->resource,'server_version');if($version<7.4){throw
new
DibiDriverException('Reflection requires PostgreSQL 7.4 and newer.');}$query="
			SELECT
				table_name AS name,
				CASE table_type
					WHEN 'VIEW' THEN 1
			...
			WHERE
				table_schema = current_schema()";if($version>=9.3){$query.="
				UNION ALL
				SELECT
					matviewname, 1
				FROM
					pg_matviews
				WHERE
					schemaname = current_schema()";}$res=$this->query($query);$tables=pg_fetch_all($res->resultSet);return$tables?$tables:array();}function
getColumns($table){$_table=$this->escape($table,dibi::TEXT);$res=$this->query("
		...
		");if(!$res->getRowCount()){$res=$this->query("
				SELECT
...
instanceof
DateTimeInterface){$value=new
DibiDateTime($value);}return$value->format($type===dibi::DATETIME?$this->fmtDateTime:$this->fmtDate);default:throw
new
InvalidArgumentException('Unsupported type.');}}function
escapeLike($value,$pos){throw
new
